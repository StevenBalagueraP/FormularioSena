<template>
  <div class="flex h-screen bg-gray-100 overflow-x-hidden">
    <!-- Sidebar -->
    <aside class="w-64 bg-gray-100 shadow-xl p-4 flex flex-col flex-shrink-0">
      <!-- Logo -->
      <div class="flex justify-center mb-3">
        <img src="/img/logo-sena.png" alt="Logo SENA" class="rounded-full border border-gray-300 "
      ...
      <nav class="flex-1">
        <ul class="space-y-2">
          <li v-for="item in menuItems" :key="item.name" @click="item.action && item.action()"
    ...
    <div class="flex-1 flex flex-col overflow-x-hidden">
      ...
      <header class="bg-white shadow-md p-4 flex justify-between items-center">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="flex justify-end p-4 bg-primary text-white rounded-s">
            <button @click="logout" class="text-white text-xs hover:text-yellow-400 transition">
              Cerrar sesión
            </button>
      ...
      <!-- CRUD Table -->
      <main class="p-6 overflow-x-hidden overflow-y-auto">
        <slot />
      </main>
    </div>
...
import CreateProgram from './Program/CreateProgram.vue';
